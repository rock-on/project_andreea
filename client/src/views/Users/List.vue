...
<div>
    ...
    <v-card>
        ...
        <v-data-table :headers="headers" :items="items" :search="search" :loading="loading" loading-text="Asteptati... Se incarca utilizatorii">
            ...
            <template v-slot:item.option="{ item }">
                <v-menu offset-y>
                    ...
                    <v-list class="pa-0">
                        <v-btn width="160" block :to="{name: constants.ROUTES.USERS_UPDATE, params: { id: item._id }}">Edit <v-icon class="ml-auto" dark>edit</v-icon>
                        </v-btn>
                        <v-btn width="160" block @click="showModal(item._id, item.fname, item.lname)">Delete <v-icon class="ml-auto" dark>delete</v-icon>
                        </v-btn>
                    </v-list>
                </v-menu>
            </template>
